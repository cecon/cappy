...
@endsection
      ...
      const sectionChunk = chunks.find(
        (c: any) => c.metadata.elementKind === "section"
      );
...
<x-alert type="error" message="Something went wrong" />
      ...
      expect(chunks.length).toBeGreaterThan(0);
      const componentChunk = chunks.find(
        (c: any) => c.metadata.elementKind === "component"
      );
...
@endforeach
      ...
      const ifDirective = chunks.find(
        (c: any) => c.metadata.elementName === "if"
      );
      expect(ifDirective).toBeDefined();
      expect(ifDirective?.metadata.elementKind).toBe("directive");

      const foreachDirective = chunks.find(
        (c: any) => c.metadata.elementName === "foreach"
      );
